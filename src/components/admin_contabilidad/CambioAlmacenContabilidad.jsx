// src/components/admin_contabilidad/CambioAlmacenContabilidad.jsx
...
export default function CambioAlmacenContabilidad() {
    ...
    const cargar = async () => {
      try {
        const [prodRes, empRes, almRes, fabRes] = await Promise.all([
          api.get(`/api/contabilidad/productos/${productoId}`),
          api.get("/api/contabilidad/empresas"),
          api.get("/api/contabilidad/almacenes"),
          api.get("/api/contabilidad/fabricantes"),
        ]);
        ...
        console.error("❌ Error cargando cambio almacén Contabilidad:", e);
      } finally {
        setLoading(false);
      }
    };
  ...
  const guardar = async () => {
    ...
    try {
      const res = await api.post("/api/contabilidad/cambios-almacen", payload);
      setToast({ type: "success", message: "✅ Cambio de almacén creado" });
      setTimeout(() => navigate(-1), 1200);
    } catch (error) {
      console.error("❌ ERROR BACKEND Contabilidad:", error.response?.data || error);
      setToast({
        type: "error",
        message:

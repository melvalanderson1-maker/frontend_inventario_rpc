...
import "./MovimientoSaldoInicial.css";

export default function MovimientoSaldoInicial({
  modoEmbedded = false,
  tipo: tipoProp,
  onGuardarFinal,
  onCancelar
  ...
  const tipo = modoEmbedded ? tipoProp : params.tipo;
  ...
  useEffect(() => {
    Promise.all([
      ...
      axios.get(`/api/motivos-movimiento?tipo=${tipo}`)
    ]).then(([p, e, a, f, m]) => {
      if (!modoEmbedded && p) {
  ...
  /* =====================================================
     ...
     - Copia de la primera si no existe
     - Nunca se marca listo automáticamente
  ===================================================== */
...
const ejecutarGuardado = async () => {
  try {
    ...
    const url =
      tipo === "entrada"
        ? "/api/compras/movimientos/entrada"
        : "/api/compras/movimientos/saldo-inicial";
    ...
    if (esProductoSimple || variantes.length === 1) {
      ...
      for (const [varianteId, obj] of Object.entries(formsPorVariante)) {
        ...
        await axios.post(url, {
          productoId: Number(varianteId),
          ...obj.data
        });
      }
    }

    showToast("✅ Movimientos creados correctamente", "success", 2000);
  ...
  const cambiarVariante = v => {
    ...
    // Persistir actual antes de salir
    persistirActual(curr => ({
      ...curr,
      data: form
    }));

...
export default function TablaCambiosAlmacenPendientesCompras({ filtro = "" }) {
  ...
  const cargar = () => {
    ...
    api
      .get("/api/compras/cambios-almacen/todos", { params: { estados } })
      .then((res) => setRows(res.data || []))
      .catch((err) => {
        console.error("❌ Error cargando cambios almacén compras:", err);
        setRows([]);
      })
  ...
  return (
    <div className="table-wrapper">
      <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 10 }}>
        <strong>Cambios de almacén - Compras</strong>

        <button
          onClick={() => setModo((m) => (m === "pendientes" ? "todos" : "pendientes"))}
          className="btn-ir"

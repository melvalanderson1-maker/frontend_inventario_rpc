...
export default function RegistrarNotas() {
  ...
  const guardarNotas = async () => {
    setSaving(true);
    try {
      const payload = { notas: alumnos.map(a => ({ usuario_id: a.id, nota: parseFloat(a.nota || 0) })) };
  ...
  return (
    <div className="registrar-notas">
      ...
      <div className="form-row">
        ...
        <input value={seccionId} onChange={(e) => setSeccionId(e.target.value)} placeholder="Sección ID" />
        <button className="btn" onClick={() => seccionId && fetchAlumnos(seccionId)}>Cargar</button>
      </div>
            ...
            alumnos.map((a, i) => (
              <div key={a.id} className="nota-row">
                <div className="alumno-info">
                  <div className="alumno-name">{a.nombre} {a.apellido_paterno}</div>
                  <div className="alumno-doc">{a.numero_documento}</div>
                </div>
                <div className="alumno-input">
                  <input type="number" step="0.01" min="0" max="100" value={a.nota} onChange={(e) => updateNota(i, e.target.value)} />
                </div>
              </div>
            ))
          }
      ...
      <div className="save-row">
        <button className="btn primary" onClick={guardarNotas} disabled={saving}>{saving ? "Guardando..." : "Guardar notas"}</button>
      </div>
    </div>
  );
}
